...
function T_Schedule() {
  ...
  const getSlotAt = (dayOfWeek, period) =>
    (schedule?.slots || []).find((s) => s.dayOfWeek === dayOfWeek && s.period === period);
          ...
          {!schedule?.slots?.length ? (
            <div className="bg-white rounded-xl shadow-sm border p-12 text-center text-gray-500">
              ...
              <p className="text-sm mt-2">
                ตารางสอนจะแสดงเมื่อมีการกำหนดรายวิชาและตารางเรียนให้คุณ
              </p>
            </div>
          ) : (
            <>
                      ...
                      <tr key={d.dow}>
                        ...
                        {PERIODS.map((p) => {
                          ...
                          return (
                            <td
                              key={p.period}
                              className={`border border-gray-200 px-2 py-2 align-top min-w-[100px] ${
                                p.isLunch ? "bg-gray-50" : ""
                              }`}
                            >
                              {slot ? (
